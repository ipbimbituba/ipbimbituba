  ...
  return (
    <>
      ...
      <nav className="w-full sticky top-0 z-[100] bg-gray-50 px-4 md:px-8 pt-5 pb-10 py-4 font-roboto">
        ...
        {/* MENU MOBILE LATERAL */}
        <ul
          className={`fixed top-0 left-0 h-full w-72 bg-white z-50 text-gray-600 pt-20 transform transition-transform duration-500 md:hidden shadow-2xl ${menuOpen ? "translate-x-0" : "-translate-x-full"
            }`}
        >
          {navItems.map((item, index) => {
                ...
                {item.submenu ? (
                  <>
                    ...
                    >
                      <span>{item.name}</span>
                      <span
                        className={`pr-6 transition-transform ${openDropdown === item.name ? "rotate-90" : ""
                          }`}
                      ></span>
                    </button>
